...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
  ...
  // (2)
  entry->foreign.addr = src; // ここわかる
  entry->foreign.port = hdr->src; // ここわかる
  entry->len = len - sizeof(*hdr); // 多分 payloadの長さ
  memcpy(entry + 1, hdr + 1, entry->len); // entry+1にデータ部分をコピーしているが、entry+1でデータ両機を指しているのか
  // 可変長なのでこうやって入れるしかないのね
