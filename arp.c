...
arp_cache_select(ip_addr_t pa)
{
  ...
  for (entry = caches; entry < tailof(caches); entry++) {
    char addr1[IP_ADDR_STR_LEN];
    errorf("pa=%s", ip_addr_ntop(entry->pa, addr1, sizeof(addr1)));
...
arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
  ...
  if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER) {
    errorf("invalid hardware address type (not eth)");
    return;
  }

  if (msg->hdr.hln != ETHER_ADDR_LEN)
  {
    errorf("invalid hardware address len (not eth)");
    return;
  }

  if (ntoh16(msg->hdr.pro) != ARP_PRO_IP)
  {
    errorf("invalid protocol address type (not eth)");
    return;
  }

  if (msg->hdr.pln != IP_ADDR_LEN)
  {
    errorf("invalid protocol address len (not eth)");
    return;
  }
  // ex 13-1
